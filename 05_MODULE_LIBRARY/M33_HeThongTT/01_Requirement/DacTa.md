...
khi bắt đầu năm kế hoạch** — vì vậy đặc tả bổ sung thực thể `MaintenancePlan` (mục 2.6); và §6.4.2
...
| `inventory_due_at` | date | tự tính | Chỉ khi `discovery_source = Phát hiện chưa kiểm kê`: hạn đưa vào vận hành = ngày lập + **30 ngày** (§6.7) |
| `network_isolated` | bool | tự quản lý | Đã ngắt khỏi mạng của Viện do không đạt cấu hình an toàn cơ sở (§6.7 bước 3) |
...
| `account_type` | enum: Cá nhân định danh / Đặc quyền – quản trị / Dịch vụ – hệ thống / **Bên thứ ba** / Dùng chung (ngoại lệ) | có | `ETV.P28` mục 5.7.1; `ETV.P33` §6.4.1 nêu bốn loại (cá nhân · đặc quyền · dịch vụ · bên thứ ba) và §6.4.3 nêu thêm **dùng chung** là ngoại lệ ⇒ enum gộp đủ 5 |
...
| `secret_location` / `secret_issuer` | text / ref User | có | **Nơi lưu** bí mật xác thực và **người có quyền cấp phát** — ghi *nơi lưu*, tuyệt đối không ghi *giá trị* (§6.4.1, quy tắc R7) |
...
| `escalated_to_ldv_at` | datetime | có **khi** `priority = Cao` | Báo cáo LĐV trong **01 giờ** (§6.5.2) |
...
| `asset_back_to_normal` | bool | có, khi đóng | Xác nhận tài sản trở lại hoạt động bình thường (§6.5.4) |
| `lesson_ref` / `no_lesson_reason` | ref → M26 / text | có, khi đóng (một trong hai) | Kết luận **có** lập bài học kinh nghiệm hay không (`ETV.P26`) — §6.5.4 |
...
| `downtime_needs` / `resource_needs` | text | có | Nhu cầu ngừng dịch vụ và nguồn lực (§6.3.1) |
...
| `scope` | enum: Toàn bộ tài khoản / Đặc quyền và dịch vụ | có | Hai chu kỳ khác nhau của §6.4.2 |
...
| `performed_by` / `reviewed_by` | ref User (QTHT) / ref User (PT.ATTT) | có | Rà soát đặc quyền cần PT.ATTT (§6.4.2 bước 5) |
...
|---|---|
| Công khai, **Nội bộ** | **Được phép**, với điều kiện thiết bị đã đăng ký trong danh mục và đủ cấu hình an toàn cơ sở (§6.2.3) |
...
thiết bị đo** — phát hiện thì ngắt kết nối và xử lý theo §6.7 (quy tắc R17).
...
§6.5.2: tài sản có `criticality = Cao`, nền tảng ManLab ngừng, hoặc ảnh hưởng hệ thống thu thập dữ
liệu đo.
...
**R7. Không lưu bí mật xác thực** (`ETV.P33` §6.1.1, §6.4.1 và Phụ lục I.1 điều kiện 7 — **cấm tuyệt
...
bài học kinh nghiệm** theo `ETV.P26` (§6.5.4).
...
trước khi PT.ATTT xem xét (§6.4.3), và mở sự cố ở M28.
...
**mục 2.3**, `ETV.P02_BaoMat.md` §6.8, `ETV.P15` · Tiêu chuẩn: ISO/IEC 27001 A.5.9, A.7.9–A.7.10,
...
| 2 | Thiết bị đầu cuối thuộc danh mục của M33 hay M27? | **Thiết bị ở P33, dữ liệu trên thiết bị ở P27**; báo cáo A.5.9 là hợp của hai | §2.2 Nguyên tắc 1 |
| 3 | Tài khoản: M33 giữ danh mục hay chỉ đọc từ M28? | M33 **giữ danh mục và thực thi** theo phiếu F28.04; phê duyệt vẫn ở P28 | §2.2 Nguyên tắc 2, §6.4 |
| 4 | Thời hạn vá lỗi bảo mật? | **07 / 30 / 90 ngày** theo mức, Nghiêm trọng có thêm mốc giảm thiểu **48 giờ** | §6.3.3 |
| 5 | BYOD có được xử lý thông tin Nội bộ không? | **Được**, nếu đã đăng ký và đủ cấu hình cơ sở; Hạn chế/Mật cần LĐV duyệt | §6.2.4 |
| 6 | Ngưỡng sự cố lặp để mở KPH? | **≥ 03 lần/90 ngày** trên cùng tài sản | §6.5.3 |
| 7 | Dịch vụ thuê ngoài kiểm kê ở đâu? | Kiểm kê tại **P33 với tư cách hạ tầng**; nếu đồng thời là nền tảng nghiệp vụ thì **đăng ký thêm ở P35 và liên kết** | §2.1 |
| 8 | Kiểm kê kỳ đầu bắt đầu từ đâu? | **Hai đợt**: trọng yếu trong **90 ngày**, phần còn lại trong **180 ngày** | §6.1.5 |
