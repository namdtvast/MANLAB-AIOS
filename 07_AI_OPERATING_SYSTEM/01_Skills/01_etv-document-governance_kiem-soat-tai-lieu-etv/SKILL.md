...
7. Khi nội dung đã chốt và cần xuất file `.docx`/`.pptx`/`.xlsx` vật lý (không chỉ Markdown), chuyển sang skill `xu-ly-van-phong` (`07_AI_OPERATING_SYSTEM/01_Skills/02_xu-ly-van-phong_tao-sua-file-van-phong`) — skill đó sở hữu toàn bộ pipeline sinh file, không dựng lại thể thức pháp lý đã chốt ở đây.
