...
Áp dụng khi văn bản đã hoàn tất phê duyệt và sẵn sàng ký chính thức (ETV.P14 §6.7, `knowledge/07_ky_so.md`).

1. **Kiểm tra điều kiện tiên quyết**: văn bản đã ở `status = Đã phê duyệt` (hoặc `Chờ phê duyệt` đã Đạt) — AI từ chối hỗ trợ nếu văn bản chưa qua đủ bước Soát xét/Phê duyệt (xem ETV.P14 §6.6).
...
5. **Xác nhận sau ký**: cập nhật `digital_signature` trong metadata, chuyển bước Ban hành/Phân phối theo ETV.P14 §6.6 (vòng đời).
