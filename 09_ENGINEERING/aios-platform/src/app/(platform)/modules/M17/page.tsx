...
type Trang = { trang?: string; trangBb?: string };

export default async function M17ListPage({ searchParams }: { searchParams: Promise<Trang> }) {
  const query: Trang = await searchParams;
  const [tongCt, tongBb, role] = await Promise.all([
    prisma.m17ReviewPlan.count(),
    prisma.m17ReviewMinutes.count(),
    getM17Role(),
  ]);
  const trangCt = chotTrang(query.trang, tongCt);
  const trangBb = chotTrang(query.trangBb, tongBb);
  const [plans, minutes] = await Promise.all([
    prisma.m17ReviewPlan.findMany({
      orderBy: { createdAt: "desc" },
      include: { createdBy: true },
      skip: boQua(trangCt),
      take: KICH_THUOC_TRANG,
    }),
    prisma.m17ReviewMinutes.findMany({
      orderBy: { createdAt: "desc" },
      include: { plan: true },
      skip: boQua(trangBb),
      take: KICH_THUOC_TRANG,
    ...
    <div className="flex flex-col gap-8">
      ...
      <section id="chuong-trinh" className="flex scroll-mt-24 flex-col gap-2">
        ...
        <div className="overflow-x-auto rounded-xl border border-border bg-surface">
          ...
          <PhanTrang path="/modules/M17" query={query} neo="#chuong-trinh" trang={trangCt} tong={tongCt} donVi="chương trình" />
        </div>
      </section>

      <section id="bien-ban" className="flex scroll-mt-24 flex-col gap-2">
        <h2 className="font-head text-sm font-bold text-ink">Biên bản xem xét</h2>
        <div className="overflow-x-auto rounded-xl border border-border bg-surface">
          <table className="w-full min-w-[36rem] text-sm">
            <thead>
              <tr>
          ...
          <PhanTrang path="/modules/M17" query={query} neo="#bien-ban" tenTham="trangBb" trang={trangBb} tong={tongBb} donVi="biên bản" />
        </div>
      </section>
    </div>
  );
