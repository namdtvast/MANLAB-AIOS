...
export default async function CRMPage({ searchParams }: { searchParams: Promise<{ q?: string; role?: string; status?: string }> }) {
  ...
  const parties = await prisma.m34Party.findMany({
    where: {
      ...
      ...(role ? { roles: { some: { roleType: role as never, active: true } } } : {}),
    },
  ...
  return <div className="flex flex-col gap-6">
    ...
    <form className="grid gap-3 rounded-xl border border-border bg-surface p-4 md:grid-cols-[minmax(16rem,1fr)_14rem_12rem_auto]" aria-label="Bộ lọc CRM">
      ...
      <label className="text-sm font-medium text-ink">Vai trò<select name="role" defaultValue={role} className="mt-1 w-full rounded-lg border border-border-strong bg-canvas px-3 py-2 font-normal"><option value="">Tất cả vai trò</option>{Object.entries(PARTY_ROLE_LABEL).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>
    ...
    <section><h2 className="mb-2 text-sm font-bold text-ink">Danh sách ({parties.length})</h2><div className="overflow-x-auto rounded-xl border border-border bg-surface"><table className="w-full min-w-[58rem] text-sm"><thead><tr><th className={th}>Mã</th><th className={th}>Tổ chức/Cá nhân</th><th className={th}>Định danh</th><th className={th}>Vai trò</th><th className={th}>Đầu mối</th><th className={th}>Trạng thái</th></tr></thead><tbody>{parties.map(p=><tr key={p.id} className="border-b border-border last:border-0 hover:bg-sunk"><td className="px-3 py-3"><Link className="font-mono text-xs font-semibold text-accent hover:underline" href={`/modules/M34/crm/${p.id}`}>{p.code}</Link></td><td className="px-3 py-3"><div className="font-medium text-ink">{p.legalName}</div><div className="text-xs text-ink-3">{PARTY_TYPE_LABEL[p.partyType]}{p.shortName ? ` · ${p.shortName}` : ""}</div></td><td className="px-3 py-3 text-ink-2">{p.taxId || "—"}</td><td className="px-3 py-3"><div className="flex flex-wrap gap-1">{p.roles.map(r=><span key={r.id} className="rounded-full bg-sunk px-2 py-0.5 text-xs text-ink-2">{PARTY_ROLE_LABEL[r.roleType]}</span>)}</div></td><td className="px-3 py-3 text-xs text-ink-2">{p.contacts[0]?.fullName || "—"}<br/>{p.contacts[0]?.phone}</td><td className="px-3 py-3"><span className="rounded-full bg-sunk px-2 py-1 text-xs font-medium text-ink-2">{PARTY_STATUS_LABEL[p.status]}</span></td></tr>)}{!parties.length&&<tr><td colSpan={6} className="px-4 py-10 text-center text-ink-3">Chưa có hồ sơ phù hợp. Kiểm tra bộ lọc hoặc tạo Tổ chức/Cá nhân mới.</td></tr>}</tbody></table></div></section>
  </div>;
}
