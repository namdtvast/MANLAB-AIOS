...
export default async function CRMPage({ searchParams }: { searchParams: Promise<{ q?: string; role?: string; status?: string; trang?: string }> }) {
  const { q = "", role = "", status = "", trang: trangRaw } = await searchParams;
  const where = {
    ...(q ? { OR: [{ legalName: { contains: q, mode: "insensitive" as const } }, { taxId: { contains: q, mode: "insensitive" as const } }, { code: { contains: q, mode: "insensitive" as const } }] } : {}),
    ...(status ? { status: status as never } : {}),
    ...(role ? { roles: { some: { roleTypeCode: role, active: true } } } : {}),
  };
  // Đếm và phân trang ở tầng DB: bộ lọc phía trên đã chạy ở DB rồi, chỉ còn phần cắt trang.
  const tong = await prisma.m34Party.count({ where });
