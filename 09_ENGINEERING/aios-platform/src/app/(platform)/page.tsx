...
// Cổng tài liệu của repo (docs/index.html) — nơi ĐỌC văn bản gốc, khác vai trò
// với nền tảng này là nơi LÀM. Dùng chung biến môi trường với CanCuBanner.tsx.
const DOCS_PORTAL =
...
function SectionHead({ eyebrow, title, lead }: { eyebrow: string; title: string; lead?: string }) {
  return (
    <div>
      ...
      {lead && <p className="mt-1 max-w-3xl text-sm text-ink-2">{lead}</p>}
    </div>
  );
}

// --- Nội dung tĩnh: câu trả lời cho "trang này là gì / cho ai / được gì" ------
// Đây là phần định vị sản phẩm, không phải dữ liệu nghiệp vụ, nên viết thẳng ở
// đây; mọi con số bên dưới đều lấy từ DB, không viết cứng.

const FEATURES = [
  {
    title: "Căn cứ pháp lý hiện ngay trên màn hình làm việc",
    body: "Mỗi trang module mở ra là thấy thủ tục viện dẫn, lần ban hành, ngày hiệu lực, chủ sở hữu quy trình, điều khoản ISO và biểu mẫu áp dụng — nạp từ manifest của MPxx, bấm mở được file gốc.",
  },
  {
  ...
  },
  {
    ...
    body: "Một người có thể là NTH ở module này và LĐP ở module khác. Quyền gán theo từng module đúng vốn từ vai trò của thủ tục đó.",
  },
  {
  ...
  },
  {
    ...
    body: "Agent phải qua đánh giá tác động (AIA) và cổng công cụ mới được chạy. AI không bao giờ tự ra kết luận đo lường cuối cùng hay tự phê duyệt chứng chỉ.",
  },
];
    ...
    <div className="flex flex-col gap-10">
      ...
      <section className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-accent">
          MANLAB-AIOS · Viện Kiểm định Công nghệ và Môi trường
        </p>
        <h1 className="max-w-3xl font-head text-2xl font-bold leading-snug text-ink sm:text-3xl">
          Nơi thủ tục của Viện được đem ra chạy, không phải nơi cất giữ tài liệu
        </h1>
        <p className="mt-3 max-w-3xl text-sm leading-relaxed text-ink-2">
          Đây là hệ điều hành doanh nghiệp của Viện: {total} module số hóa đúng {total} thủ tục
          trong hệ thống quản lý, dùng chung một cơ sở dữ liệu, một cách đăng nhập và một bộ quy
          tắc. Mỗi màn hình gắn thẳng với một thủ tục đã ban hành — làm việc trên đây tức là làm
          đúng thủ tục, và bằng chứng tuân thủ sinh ra ngay trong lúc làm.
        </p>
        <div className="mt-5 flex flex-wrap items-center gap-2.5">
          {activeModules[0] && (
            <Link
              href={`/modules/${activeModules[0].code}`}
              className="inline-flex items-center gap-1.5 rounded-lg bg-accent px-3.5 py-2 text-sm font-semibold text-accent-ink transition-opacity hover:opacity-90"
            >
              Vào module đang vận hành
              <Arrow />
            </Link>
          )}
          <a
            ...
            className="inline-flex items-center gap-1.5 rounded-lg border border-border-strong px-3.5 py-2 text-sm font-semibold text-ink transition-colors hover:bg-sunk"
          >
            Mở cổng tài liệu gốc
          </a>
        </div>
      </section>
      ...
      {/* 3. Khác gì những nơi khác mà Viện đang dùng */}
      <section className="flex flex-col gap-4">
        <SectionHead
          eyebrow="Khác biệt"
          title="Trang này khác gì cổng tài liệu và ổ chia sẻ?"
      ...
      <section className="flex flex-col gap-4">
        <SectionHead
          eyebrow="Đối tượng"
          title="Ai làm việc trên trang này?"
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {AUDIENCE.map((a) => (
            <div key={a.role} className="rounded-xl border border-border bg-surface p-4">
              <p className="font-head text-sm font-bold text-ink">{a.role}</p>
              <p className="mt-1.5 text-sm leading-relaxed text-ink-2">{a.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="flex flex-col gap-4">
        <SectionHead
          eyebrow="Giá trị"
          title="Trang này đóng góp gì — và thiếu nó thì sao?"
        />
      ...
      <section className="flex flex-col gap-4">
        <SectionHead
          eyebrow="Bắt đầu"
          title="Bản đồ nghiệp vụ"
          lead="Chấm xanh là module đã vận hành thật trên nền tảng; các module còn lại đang ở giai đoạn đặc tả và mở trang giới thiệu."
        />
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {groups.map((g) => (
      ...
      {/* 8. Nói thẳng tình trạng thật — tránh nhầm "đã dựng" với "đã hoàn thiện" */}
      <section className="rounded-xl border border-border bg-sunk p-4 text-sm text-ink-2">
        ...
        <p className="mt-1.5 max-w-3xl leading-relaxed">
          ...
          module luôn nói rõ phần nào đã có, phần nào chưa — không quy tròn &ldquo;đã dựng&rdquo;
          thành &ldquo;đã hoàn thiện&rdquo;.
        </p>
      </section>
    </div>
  );
