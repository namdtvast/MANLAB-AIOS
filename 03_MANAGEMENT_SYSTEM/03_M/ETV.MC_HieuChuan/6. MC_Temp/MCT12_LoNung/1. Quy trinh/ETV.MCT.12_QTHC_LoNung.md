---
id: ETV.MCT 12
title: "Lò nung — Quy trình hiệu chuẩn"
type: Quy-trinh
owner: "Viện Kiểm định Công nghệ và Môi trường"
department: "Phòng Đo lường Chất lượng"
prepared_by: ""
prepared_date: "22/04/2026"
reviewed_by: "Trần Thị Hoa"
reviewed_date: "22/04/2026"
approved_by: "Nguyễn Hoàng Giang"
approved_date: "22/04/2026"
process: ""
effective_date: "22/04/2026"
revision: "03"
status: Da-ban-hanh
keywords: [lò nung, furnace, độ ổn định, độ đồng đều, ITS-90, hiệu chuẩn]
related_documents: []
iso_clause: ["ISO/IEC 17025:2017"]
legal_basis: ["ĐLVN 113:2003", "ĐLVN 138:2004", "ĐLVN 127:2003", "ĐLVN 131:2003", "VMI-CP 17:2013"]
ai_tags: [calibration-procedure, furnace, uncertainty-budget]
knowledge_category: HTQL-noi-bo
permission: Noi-bo
retention: "Theo ETV.P.F 14.06"
digital_signature: "Nguyễn Hoàng Giang"
source: "Viện Kiểm định Công nghệ và Môi trường (ETV) — chuyển đổi định dạng từ bản PDF gốc `ETV.MCT 12_Lo nung_V3.pdf`"
supersedes: "ETV.MCT 12 lần ban hành 02 (19/05/2020, sửa đổi 22/04/2023 và 15/06/2024)"
superseded_by: null
---
# LÒ NUNG – QUY TRÌNH HIỆU CHUẨN

*Furnaces – Calibration Procedure*

|                   |                     |
| ----------------- | ------------------- |
| **Mã số**         | ETV.MCT 12          |
| **Lần ban hành**  | 03                  |
| **Ngày ban hành** | 22/04/2026          |
| **Biên soạn**     | *(bản gốc để trống)* |
| **Soát xét**      | Trần Thị Hoa        |
| **Phê duyệt**     | Nguyễn Hoàng Giang  |

> **Tình trạng bản này:** Bản chuyển đổi định dạng (PDF → Markdown) từ file gốc `ETV.MCT 12_Lo nung_V3.pdf` do Viện ban hành, phục vụ tra cứu trên ManLab. Khối chữ ký trong bản gốc để trống, chưa có tên người biên soạn.
>
> *Ghi chú của bản chuyển đổi:* công thức "ĐKĐB tổng hợp" tại mục 7.4.2 trong bản gốc ghi `u_c = √(u_c² + u_bk²)` — có dấu hiệu lỗi đánh máy (định nghĩa `u_c` bằng chính nó), theo mạch tính toán (tổ hợp `u_ch` và thiết bị `u_bk`) hợp lý là `u_c = √(u_ch² + u_bk²)`. Giữ nguyên văn công thức gốc, chú thích khả năng lỗi tại đây.

> **Chú ý:** Văn bản nội bộ nghiêm cấm cung cấp cho bên ngoài khi chưa có sự đồng ý của Lãnh đạo Viện.

## NHỮNG THAY ĐỔI ĐÃ CÓ

| Thời gian  | Nội dung thay đổi                              | Lần ban hành |
| ---------- | ---------------------------------------------------- | ------------ |
| 19/05/2020 | Ban hành lần thứ nhất                                 | 01           |
| 22/04/2023 | Sửa đổi theo ý kiến chuyên gia đánh giá               | 02           |
| 15/06/2024 | Sửa đổi theo ý kiến chuyên gia Quatest 1              | 02           |
| 22/04/2026 | Ban hành lần thứ ba                                   | 03           |

---

## 1. Phạm vi áp dụng

Văn bản kỹ thuật này quy định quy trình hiệu chuẩn các loại lò nung dải nhiệt độ đến 1.200 °C theo Thang nhiệt độ quốc tế 1990 (ITS-90).

Quy trình này không áp dụng để hiệu chuẩn đối với các loại lò nung sử dụng sóng điện từ để gia nhiệt.

Quy trình này áp dụng đối với nhân viên của Viện Kiểm định Công nghệ và Môi trường (sau đây gọi tắt là PTN) khi hiệu chuẩn PTĐ nói trên.

## 2. Giải thích từ ngữ

Các từ ngữ trong văn bản này được hiểu như sau:

- **Lò nhiệt (Lò nung):** là thiết bị sử dụng điện để gia nhiệt khoang làm việc, có hệ thống đo và điều khiển nhiệt độ theo giá trị đặt. Khoang làm việc có thể sử dụng hoặc không sử dụng quạt đối lưu.
- **Hiệu chuẩn:** là hoạt động xác định, thiết lập mối quan hệ giữa giá trị đo của chuẩn đo lường, phương tiện đo với giá trị đo của đại lượng cần đo.
- **Hiệu chỉnh:** là tập hợp các thao tác được tiến hành trên hệ thống đo để cho ra số chỉ đã quy định tương ứng với giá trị đã cho của đại lượng được đo.
- **Độ không đảm bảo đo (ĐKĐB):** thông số gắn với kết quả của phép đo, đặc trưng cho sự phân tán của các giá trị có thể quy cho đại lượng đo một cách hợp lý.

## 3. Các phép hiệu chuẩn

Phải lần lượt tiến hành các phép kiểm tra ghi trong Bảng 1.

**Bảng 1**

| TT  | Tên phép hiệu chuẩn                            | Theo điều, mục của quy trình |
| --- | ---------------------------------------------------- | -------------------------------- |
| 1   | Kiểm tra bên ngoài                                    | 7.1                               |
| 2   | Kiểm tra kỹ thuật                                     | 7.2                               |
| 3   | Kiểm tra đo lường — xác định số hiệu chính của lò nung | 7.3.4.1                       |
|     | — kiểm tra độ ổn định                                 | 7.3.4.2                           |
|     | — kiểm tra độ đồng đều                                | 7.3.4.3                           |
| 4   | Tính toán độ không đảm bảo đo                         | 7.4                               |
| 5   | Xử lý chung                                           | 8                                 |

## 4. Phương tiện phục vụ hiệu chuẩn

Phương tiện hiệu chuẩn được ghi trong Bảng 2.

**Bảng 2**

| TT  | Phương tiện hiệu chuẩn              | Đặc trưng kỹ thuật                                                                                                                          |
| --- | -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Chuẩn đo lường**                     | Các chuẩn đo lường phải được liên kết chuẩn theo quy định và độ không đảm bảo đo của tổ hợp chuẩn so với lò nung phải thỏa mãn tỉ số truyền chuẩn ≤ 1/3 |
| 1.1 | Nhiệt kế chuẩn                         | Phạm vi đo phù hợp với dải đo làm việc của lò nung; độ không đảm bảo đo phù hợp với dẫn xuất chuẩn và liên kết chuẩn với hệ thống chuẩn quốc gia |
| 1.2 | Thiết bị chỉ thị chuẩn                 | Phạm vi đo phù hợp với dải đo làm việc của lò nung; độ phân giải ≤ 0,1 °C; độ không đảm bảo đo của tổ hợp chuẩn so với thiết bị thỏa mãn tỉ số truyền chuẩn ≤ 1/3 |
| 2   | **Phương tiện khác**                   |                                                                                                                                                    |
| 2.1 | Thiết bị đo nhiệt độ và độ ẩm môi trường | Dải đo: Nhiệt độ (0 ÷ 50) °C; Độ ẩm tương đối (0 ÷ 100) %RH. Độ phân giải: Nhiệt độ 1 °C; Độ ẩm 1 %RH; liên kết chuẩn với hệ thống chuẩn quốc gia |
| 3   | **Phương tiện phụ**                    |                                                                                                                                                    |
| 3.1 | Đồng hồ đo thời gian                   | —                                                                                                                                                  |
| 3.2 | Găng tay, dung dịch làm sạch, vải cotton | —                                                                                                                                                |
| 3.3 | Hệ thống gá lắp nhiệt kế               | —                                                                                                                                                  |

## 5. Điều kiện hiệu chuẩn

Khi tiến hành hiệu chuẩn phải đảm bảo các điều kiện sau đây:

- Nhiệt độ: (25 ± 5) °C;
- Độ ẩm tương đối: (40 ÷ 70) %RH.

> *Lưu ý:* Điều kiện môi trường hiệu chuẩn hiện trường chỉ cần thoả mãn với yêu cầu sử dụng của lò nung cần hiệu chuẩn.

## 6. Chuẩn bị hiệu chuẩn

Trước khi tiến hành hiệu chuẩn phải thực hiện các công việc chuẩn bị sau đây:

- Kiểm tra tình trạng hoạt động của lò nung theo hướng dẫn sử dụng của nhà sản xuất;
- Đặt các đầu đo của nhiệt kế chuẩn vào lò nung, có thể đưa các đầu đo theo lối vào có sẵn của lò nung, sau đó dùng vật liệu cách nhiệt lấp kín hoặc đưa các đầu đo theo cửa lò nung nếu cửa có thể đóng kín được khi hoạt động;
- Các đầu đo nhiệt độ chuẩn được đặt cố định phân bố đều trong lò, phân bố theo các góc của một hình khối lăng trụ, các điểm đo không gian của đầu đo nhiệt độ chuẩn được đặt cách các thành của lò nung một khoảng cách từ 50 mm đến 60 mm và 1 đầu đo nhiệt độ chuẩn được đặt cố định tại tâm hình học của lò (xem Hình 1);
- Lắp các đầu đo nhiệt độ chuẩn vào thiết bị chỉ thị (hoặc tự ghi) chuẩn theo đúng vị trí các kênh (xem số ghi trên đầu của đầu đo nhiệt độ chuẩn)/ghi nhiệt độ theo các kênh tương ứng.

*(Hình 1 trong bản gốc minh họa vị trí đặt các đầu đo nhiệt kế chuẩn — không tái tạo lại trong bản chuyển đổi này, xem bản PDF gốc.)*

Số lượng các đầu đo là 3, 5 hoặc 7 phụ thuộc vào thể tích của lò nung:

- Thể tích buồng nhiệt ≤ 0,5 m³: số đầu đo nhiệt độ chuẩn là 3;
- Thể tích buồng nhiệt (0,5 ÷ 1) m³: số đầu đo nhiệt độ chuẩn là 5;
- Thể tích buồng nhiệt > 1 m³: số đầu đo nhiệt độ chuẩn là 7.

## 7. Tiến hành hiệu chuẩn

### 7.1. Kiểm tra bên ngoài

Kiểm tra bằng mắt để xác định sự phù hợp của PTĐ đối với các yêu cầu quy định trong tài liệu kỹ thuật, hình dáng kích thước, ký nhãn hiệu, nguồn nuôi, hiển thị, tài liệu và phụ kiện kèm theo.

### 7.2. Kiểm tra kỹ thuật

Phải kiểm tra kỹ thuật theo các yêu cầu sau đây:

- Kiểm tra tình trạng hoạt động của lò nung khi cung cấp điện áp danh định được ghi trên nhãn;
- Hệ điều khiển các chức năng hoạt động tốt;
- Bộ chỉ thị nhiệt độ hoạt động ổn định, không có hiện tượng thay đổi đột ngột, biến động. Đối với nhiệt kế chỉ thị hiện số, các số hiển thị phải rõ nét, không bị mờ hoặc mất nét. Đối với nhiệt kế chỉ thị tương tự, vạch chia phải còn đầy đủ, không bị nhòe hoặc mất chữ số, kim chỉ thị không bị ma sát hoặc kẹt kim;
- Các bộ phận khác của lò nung hoạt động bình thường;
- Nếu lò nung có cửa phải đảm bảo độ kín, không bị cong vênh, nứt vỡ;
- Nếu lò nung có quạt đối lưu, kiểm tra quạt phải hoạt động bình thường.

### 7.3. Kiểm tra đo lường

Lò nung cần hiệu chuẩn được kiểm tra đo lường theo trình tự nội dung, phương pháp và yêu cầu sau đây:

#### 7.3.1. Phương pháp

Kiểm tra đo lường được thực hiện bằng phương pháp so sánh trực tiếp giá trị nhiệt độ của PTĐ chuẩn với giá trị nhiệt độ của lò nung cần hiệu chuẩn.

#### 7.3.2. Quy định chung

- **7.3.2.1.** Số điểm nhiệt độ kiểm tra phải được chia đều trong dải nhiệt độ (không ít hơn 03 điểm) hoặc theo yêu cầu của cơ sở sử dụng. Thực hiện hiệu chuẩn từ điểm nhiệt độ thấp đến điểm nhiệt độ cao.
- **7.3.2.2.** Các phép đo nhiệt độ được thực hiện khi nhiệt độ của nhiệt kế chuẩn và nhiệt độ của lò nung đã ổn định trong 30 phút. Ghi kết quả đo không ít hơn 10 lần tại mỗi điểm nhiệt độ kiểm tra.

#### 7.3.3. Trình tự kiểm tra tại một điểm nhiệt độ

- **7.3.3.1.** Đặt nhiệt độ của lò nung tương ứng với giá trị nhiệt độ đầu tiên cần kiểm tra.
- **7.3.3.2.** Sau khi nhiệt độ đã ổn định (theo mục 7.3.2.2), đọc số chỉ nhiệt độ của bộ hiển thị lò nung và số chỉ của các nhiệt kế chuẩn (từ nhiệt kế chuẩn ở vị trí 1 đến vị trí k). Trình tự đọc theo thứ tự:

  Lò nung → ch1 → ch2 → ch3 … → chk (ttn → t1 → t2 → t3 … → tk), với k = 3; 5 hoặc 7 theo thể tích của lò nung.

  Trong đó: ch1, ch2 … chk là nhiệt kế chuẩn thứ 1, thứ 2, … thứ k (°C); t1, t2 … tk là nhiệt độ của nhiệt kế chuẩn thứ 1, thứ 2, … thứ k (°C); ttn là nhiệt độ đọc trên bộ chỉ thị của lò nung (°C).

  Số lượt đọc tại mỗi điểm nhiệt độ kiểm tra không ít hơn 10 lần.

- **7.3.3.3.** Lần lượt tiến hành đo tương tự đối với các điểm nhiệt độ kiểm tra tiếp theo cho đến điểm nhiệt độ kiểm tra cuối cùng.

#### 7.3.4. Xử lý kết quả hiệu chuẩn

##### 7.3.4.1. Xác định số hiệu chính lò nung

Số hiệu chính của lò nung tại mỗi điểm nhiệt độ kiểm tra được tính theo công thức:

`Δt = t_ch - t_ln`

Trong đó `t_ch`: giá trị trung bình của tổ hợp chuẩn, được tính theo công thức:

`t_ch = (1/k)Σ(j=1→k) t_j`

`t_j`: giá trị trung bình của mỗi nhiệt kế chuẩn (chỉ thị chuẩn), tính theo công thức:

`t_j = (1/n)Σ(i=1→n) (t_j ± ∂ t_j)_i`

`∂t_j`: số hiệu chính của nhiệt kế chuẩn thứ j tại điểm nhiệt độ kiểm tra (xem trong giấy chứng nhận hiệu chuẩn); `n`: số lần đo của mỗi nhiệt kế chuẩn tại mỗi điểm nhiệt độ kiểm tra.

`t_ln`: giá trị trung bình của chỉ thị lò nung tại mỗi điểm nhiệt độ kiểm tra, tính theo công thức:

`t_ln = (1/n)Σ(i=1→n) t_i`

`t_i`: giá trị lò nung tại lần đo thứ i; `n`: số lần đo tại mỗi điểm nhiệt độ kiểm tra.

> *Ghi chú:* Số hiệu chính của lò nung (∆t) tại mỗi điểm nhiệt độ kiểm tra không được vượt quá giá trị sai số tương ứng với độ chính xác của lò nung.

##### 7.3.4.2. Xác định độ ổn định của lò nung

Độ ổn định của lò nung tại một điểm nhiệt độ được xác định như sau:

`δt_od = ± (1/2) max(t_ch max,j - t_ch min,j) j: 1,2,…,k`

Trong đó: `δt_od`: độ ổn định của lò nung tại nhiệt độ kiểm tra; `t_ch max,j`, `t_ch min,j`: nhiệt độ cao nhất và thấp nhất của nhiệt kế chuẩn thứ j tại điểm nhiệt độ kiểm tra.

##### 7.3.4.3. Xác định độ đồng đều của lò nung

Độ đồng đều của lò nung tại một điểm nhiệt độ được xác định như sau:

`δt_dd = ± (1/2) max[max(t_ch j) - min(t_ch i)] j ≠ i; j,i: 1,2,…,k`

Trong đó: `δt_dd`: độ đồng đều của lò nung; `max(t_ch j)`, `min(t_ch i)`: nhiệt độ trung bình lớn nhất và nhỏ nhất trong k nhiệt kế chuẩn tại các vị trí đo khác nhau.

### 7.4. Tính toán độ không đảm bảo đo (tính cho mỗi điểm kiểm tra)

#### 7.4.1. Các yếu tố gây ra ĐKĐBĐ bao gồm

- Thiết bị chuẩn nhiệt độ;
- Lò nung cần hiệu chuẩn;
- Nhân viên đo/hiệu chuẩn.

#### 7.4.2. Tính toán ĐKĐB của các yếu tố ảnh hưởng tại tất cả các điểm hiệu chuẩn

ĐKĐB của phép hiệu chuẩn lò nung được tính toán từ các sai số ảnh hưởng đến các phép đo nhiệt độ khi hiệu chuẩn, được chia thành hai loại: độ không đảm bảo đo của tổ hợp chuẩn và độ không đảm bảo đo của lò nung, tính toán trong toàn dải đo, cụ thể như sau:

**a) Độ không đảm bảo đo của tổ hợp chuẩn:**

`u_ch = √(u_ch1² + u_ch2²)`

**ĐKĐB của nhiệt kế chuẩn (u_ch1):**

`u_ch1 = (U_95/2)`

Với `U_95`: ĐKĐB mở rộng của nhiệt kế chuẩn, lấy từ giấy chứng nhận hiệu chuẩn.

**ĐKĐB do độ tản mạn kết quả đo tại mỗi điểm HC của nhiệt kế chuẩn (u_ch2):**

`u_ch2 = √(Σ(j=1→k) u_ch2,j²) k: số vị trí dây đo`

Với `u_ch2,j` là ĐKĐB chuẩn loại A của nhiệt kế chuẩn thứ j:

`u_ch2,j = √(S_j²/n)`

Trong đó `S_j` là độ lệch chuẩn của nhiệt kế chuẩn thứ j, tính cho n lần đọc:

`S_j = √((Σ(i=1→n) (t_i,j - t_j)²)/(n-1))`

- `n`: số lần đọc tại mỗi điểm;
- `t_i,j`: lần đọc thứ i của nhiệt kế chuẩn thứ j;
- `t_j`: nhiệt độ trung bình tại điểm kiểm tra của nhiệt kế chuẩn thứ j.

**b) Độ không đảm bảo đo của thiết bị:**

`u_bk = √(u_bk1² + u_bk2² + u_bk3² + u_bk4²)`

**ĐKĐB do độ tản mạn kết quả đo tại mỗi điểm HC của bộ chỉ thị nhiệt (u_bk1):**

`u_bk1 = √(S_j²/n)`

Trong đó `s_j` là độ lệch chuẩn tại điểm đo thứ j, `n` là số lần đọc tại mỗi điểm đo:

`s_j = √((Σ(1→n) (t_i - t)²)/(n-1))`

- `n`: số lần đọc tại mỗi điểm kiểm tra;
- `t_i`: lần đọc thứ i của lò nung;
- `t`: nhiệt độ trung bình tại điểm kiểm tra của lò nung.

**Độ không đảm bảo đo tính theo độ ổn định (u_bk2):**

`u_bk2 = (δt_od/√(3))`

Trong đó: `δt_od`: độ ổn định của lò nung tại nhiệt độ kiểm tra.

**Độ không đảm bảo đo tính theo độ đồng đều (u_bk3):**

`u_bk3 = (δt_dd/√(3))`

Trong đó: `δt_dd`: độ đồng đều của lò nung.

**Độ không đảm bảo đo theo độ phân giải của chỉ thị lò nung (u_bk4):**

Đối với chỉ thị tương tự:

`u_bk4 = (d/3√(3))`

Trong đó `d` là giá trị độ chia của lò.

Đối với chỉ thị hiện số:

`u_bk4 = (d/2√(3))`

Trong đó `d` là độ phân giải của thiết bị gia nhiệt.

**ĐKĐB tổng hợp:** Độ không đảm bảo đo liên hợp là đại lượng được xác định từ tổ hợp chuẩn và lò nung:

`u_c = √(u_ch² + u_bk²)`

**ĐKĐB mở rộng:** Độ không đảm bảo đo mở rộng (U95) là đại lượng xác định miền giá trị phân bố bao quanh kết quả đo:

`U_95 = k × u_c`

Với k = 2 là hệ số bao phủ tương ứng với mức độ tin cậy 95 % C.L.

## 8. Xử lý chung

- **8.1.** Thiết bị sau khi hiệu chuẩn được dán tem, cấp giấy chứng nhận hiệu chuẩn kèm theo kết quả hiệu chuẩn.
- **8.2.** Chu kỳ hiệu chuẩn được khuyến nghị là 12 tháng.

## 9. Phụ lục

Biên bản hiệu chuẩn lò nung (`ETV.MCT 12.01`).

## TÀI LIỆU THAM KHẢO

- ISO/IEC 17025:2017: Yêu cầu chung về năng lực của phòng thử nghiệm và hiệu chuẩn.
- ĐLVN 113:2003: Yêu cầu về nội dung và trình bày văn bản kỹ thuật đo lường Việt Nam.
- ĐLVN 138:2004 "Nhiệt kế chỉ thị hiện số và tương tự - Quy trình hiệu chuẩn", Tổng cục Tiêu chuẩn – Đo lường – Chất lượng, 2004.
- ĐLVN 127:2003 "Tủ xác định nhu cầu oxi sinh hóa (BOD) - Quy trình hiệu chuẩn", Tổng cục Tiêu chuẩn – Đo lường – Chất lượng, 2003.
- ĐLVN 131:2003 "Hướng dẫn đánh giá độ không đảm bảo đo", Tổng cục Tiêu chuẩn – Đo lường – Chất lượng, 2003.
- VMI-CP 17:2013 "Tủ nhiệt - Quy trình hiệu chuẩn", Viện Đo lường Việt Nam, 2013.
