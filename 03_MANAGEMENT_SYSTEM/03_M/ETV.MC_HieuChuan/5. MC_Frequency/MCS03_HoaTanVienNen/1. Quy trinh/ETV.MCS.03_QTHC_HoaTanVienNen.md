...
`s(V_tb) = √((Σ(1→n) (V_tbj - V̄_tb)²)/(n-1)) , rpm  (11)`

- Tính độ không đảm bảo `u_A` do đo lặp n lần:

`u_A2 = √((Σ(1→n) (V_tbj - V̄_tb)²)/n(n-1)) , rpm  (12)`
...
`T% = (A_t/A_s) × (W_s/Dose) × D × P × R × F_DS × 100  (39)`
...
`u_T = T √((u_At²/A_t²) + (u_Ast²/A_st²) + (u_Wst²/W_st²) + (u_dose²/dose²) + (u_P²/P²) + (u_D²/D²) + (u_R²/R²) + (u_FDS²/F_DS²))  (40)`

Trong đó, các thành phần độ không đảm bảo đo:

##### 7.4.4.1. ĐKĐB từ đo độ hấp thụ của dung dịch thử bằng thiết bị đo quang, u_At

`u_At = √(u_At,straylight² + u_At,Linear² + u_At,Acc² + u_At,std²)  (41)`

hoặc:

`u_At = √((U_At²/k²) + u_At,std²)  (42)`
...
`u_Ast = √(u_Ast,straylight² + u_Ast,Linear² + u_Ast,Acc² + u_Ast,std²)  (43)`

hoặc:

`u_Ast = √((U_Ast²/k²) + u_Ast,std²)  (44)`
...
`u_Ci = C_i × √((u_flask/V_flask)² + (u_pipet/V_pipet)² + ((u_Ci-1)/(C_i-1))²) (mg/L)`
