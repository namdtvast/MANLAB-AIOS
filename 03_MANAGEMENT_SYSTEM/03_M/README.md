...
| [`ETV.MV_KiemDinh/1. MV_Phy_Che_Water/`](ETV.MV_KiemDinh/1.%20MV_Phy_Che_Water/) | **14 quy trình kiểm định lĩnh vực Hoá lý (nước)** — văn bản ĐLVN quốc gia do ETV áp dụng trực tiếp, mỗi quy trình một thư mục Cấp 3 với `1. Quy trinh/`; chuyển đổi `.md` từ bản PDF ban hành, không lưu bản `.pdf` gốc trong repo. **Phương tiện đo đơn lẻ:** `MVW_DLVN31_pH` (ĐLVN 31:2017, soát xét lần 2 — thay thế ĐLVN 31:2001), `MVW_DLVN80_TDS` (ĐLVN 80:2017, soát xét lần 1 — thay thế ĐLVN 80:2002), `MVW_DLVN274_EC` (ĐLVN 274:2014 — độ dẫn điện), `MVW_DLVN275_DoDuc` (ĐLVN 275:2014 — độ đục của nước), `MVW_DLVN276_DO` (ĐLVN 276:2014 — ôxy hòa tan, chuẩn độ Winkler), `MVW_DLVN372_UVVis` (ĐLVN 372:2020 — quang phổ tử ngoại-khả kiến; bản gốc do Ban kỹ thuật TC 14 *Phương tiện đo quang học* biên soạn, ETV xếp vào lĩnh vực Hoá lý (nước) theo tiền tố `MVW_` của hồ sơ gốc). **Trạm quan trắc môi trường nước (bộ ĐLVN 2021):** `MVW_DLVN381_pHTram`, `MVW_DLVN382_TDSTram`, `MVW_DLVN383_ECTram`, `MVW_DLVN384_DoDucTram`, `MVW_DLVN385_DOTram`, `MVW_DLVN386_AmoniTram`, `MVW_DLVN387_CODTram`, `MVW_DLVN388_TSSTram` — cùng do Trung tâm Quan trắc môi trường miền Bắc biên soạn, Tổng cục Môi trường đề nghị. Biên bản kiểm định của cả 14 quy trình là **Phụ lục của chính văn bản ĐLVN**, đã transcribe trong file quy trình; ETV chưa lập biểu mẫu `ETV.MVW.F*` riêng nên chưa tạo thư mục `2. Bieu mau/`. **Lưu ý chất lượng nguồn:** 5 bản PDF cũ (ĐLVN 31, 80, 274, 275, 276) dùng phông chữ thiếu ánh xạ Unicode làm trích xuất trực tiếp mất chữ và sai ký hiệu (`µ`→`P`, `°`→`q`, `δ`→`G`); đã phục hồi bằng đối chiếu **OCR tiếng Việt** (tesseract, 300 dpi) — OCR bổ khuyết được 4 giá trị định lượng mà trích xuất trực tiếp làm mất: độ ẩm `≤ 80 %RH` (ĐLVN 31, 80) và hệ số `1/2` trong độ không đảm bảo đo của chuẩn (ĐLVN 274, 275, 276). Mỗi file có khối cảnh báo riêng; **cần LĐP soát xét đối chiếu bản in gốc** trước khi coi là bản tham chiếu chính thức thay thế PDF |
